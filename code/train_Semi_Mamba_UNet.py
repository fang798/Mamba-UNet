# CUDA_VISIBLE_DEVICES=1 python train_Semi_Mamba_UNet.py --model1_type vim --consistency_rampup 500 --model1_lr 0.05 --model2_lr 0.01 --consistency 0.5 --exp Thyroid/Semi_Mamba_UNet --max_iterations 10000 --batch_size 32 --labeled_bs 16 --num_classes 2
# CUDA_VISIBLE_DEVICES=2 python train_Semi_Mamba_UNet.py --model1_type cranet --consistency_rampup 500 --model1_lr 0.01 --model2_lr 0.01 --consistency 0.5 --exp Thyroid/Semi_Mamba_UNet --max_iterations 10000 --batch_size 32 --labeled_bs 16 --num_classes 2
# CUDA_VISIBLE_DEVICES=2 python train_Semi_Mamba_UNet.py --model1_type cranet --model2_type cranet --consistency_rampup 500 --model1_lr 0.01 --model2_lr 0.01 --consistency 0.5 --exp Thyroid/Semi_Mamba_UNet --max_iterations 10000 --batch_size 32 --labeled_bs 16 --num_classes 2

# --consistency_rampup：ramp 总共要走的步数，默认200，即200步从0逐步到1；
# consistency_weight = get_current_consistency_weight(iter_num // 150)，每 150 个 iteration 才让 ramp 走一步
...
parser.add_argument('--model1_lr', type=float, default=None,
                    help='learning rate for model1 (defaults to base_lr)')
parser.add_argument('--model2_lr', type=float, default=None,
                    help='learning rate for model2 (defaults to base_lr)')
...
parser.add_argument('--consistency', type=float,
                    default=0.5, help='consistency')    # default=0.5
parser.add_argument('--consistency_rampup', type=float,
                    default=200.0, help='consistency_rampup')
parser.add_argument('--model1_type', type=str, default='unet',
                    choices=['unet', 'vim', 'cranet'], help='model1 type: unet, vim, or cranet')
parser.add_argument('--model2_type', type=str, default='vim',
                    choices=['vim', 'cranet'], help='model2 type: vim or cranet')
...
def train(args, snapshot_path):
    ...
    model1_base_lr = args.model1_lr if args.model1_lr is not None else base_lr
    model2_base_lr = args.model2_lr if args.model2_lr is not None else base_lr
    ...
    if args.model1_type == 'unet':
        model1 = UNet(in_chns=1, class_num=num_classes).cuda()
    elif args.model1_type == 'cranet':
        ...
        model1 = CRANetWrapper().cuda()
    else:
        model1 = ViM_seg(config, img_size=args.patch_size,
                         num_classes=args.num_classes).cuda()
        model1.load_from(config)

    if args.model2_type == 'cranet':
        CRANet = _load_cranet_class()
    ...
    logging.info(
        "Total slices is: %d, labeled slices is: %d, unlabeled slices is: %d",
    ...
    optimizer1 = optim.SGD(model1.parameters(), lr=model1_base_lr,
                           momentum=0.9, weight_decay=0.0001)
    optimizer2 = optim.SGD(model2.parameters(), lr=model2_base_lr,
                           momentum=0.9, weight_decay=0.0001)
    ...
    logging.info(
        "train_params: base_lr=%s model1_lr=%s model2_lr=%s batch_size=%s labeled_bs=%s max_iterations=%s patch_size=%s",
        base_lr, model1_base_lr, model2_base_lr, batch_size, args.labeled_bs, max_iterations, args.patch_size
    )
    ...
    for epoch_num in iterator:
        epoch_iterator = tqdm(trainloader, ncols=70, leave=False)
        for i_batch, sampled_batch in enumerate(epoch_iterator):
            ...
            # consistency_weight = get_current_consistency_weight(iter_num // 25)
            consistency_weight = args.consistency * ramps.linear_rampup(
                iter_num // 25, args.consistency_rampup
            )
            ...
            lr1_ = model1_base_lr * (1.0 - iter_num / max_iterations) ** 0.9
            lr2_ = model2_base_lr * (1.0 - iter_num / max_iterations) ** 0.9
            for param_group in optimizer1.param_groups:
                param_group['lr'] = lr1_
            for param_group in optimizer2.param_groups:
                param_group['lr'] = lr2_

            writer.add_scalar('lr/model1', lr1_, iter_num)
            writer.add_scalar('lr/model2', lr2_, iter_num)
            writer.add_scalar(
            ...
            logging.info(
                "iter=%d lr1=%.6f lr2=%.6f cons=%.4f loss1=%.6f loss2=%.6f pseudo1=%.6f pseudo2=%.6f con1=%.6f model1=%.6f model2=%.6f",
                log_items["iter"], lr1_, lr2_, log_items["consistency_weight"],
                log_items["loss1"], log_items["loss2"], log_items["pseudo_supervision1"],
                log_items["pseudo_supervision2"], log_items["con1"],
                log_items["model1_loss"], log_items["model2_loss"],
            )
            epoch_iterator.set_postfix(
                iter=iter_num,
                lr1=f"{lr1_:.6f}",
                lr2=f"{lr2_:.6f}",
                cons=f"{consistency_weight:.4f}",
                model1=f"{model1_loss.item():.4f}",
                model2=f"{model2_loss.item():.4f}",
            )
            if iter_num % 50 == 0:
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                ...
                if performance1 > best_performance1:
                    ...
                    save_best = os.path.join(
                        snapshot_path,
                        '{}_best_model1_iter_{}.pth'.format(args.model, iter_num),
                    )
                ...
                if performance2 > best_performance2:
                    ...
                    save_best = os.path.join(
                        snapshot_path,
                        '{}_best_model2_iter_{}.pth'.format(args.model, iter_num),
                    )
            ...
            if iter_num % 3000 == 0:
                save_mode_path = os.path.join(
                    snapshot_path, 'time_model1_iter_' + str(iter_num) + '.pth')
                torch.save(model1.state_dict(), save_mode_path)
                logging.info("save model1 to {}".format(save_mode_path))

                save_mode_path = os.path.join(
                    snapshot_path, 'time_model2_iter_' + str(iter_num) + '.pth')
                torch.save(model2.state_dict(), save_mode_path)
...
if __name__ == "__main__":
    ...
    # snapshot_path = "../run_result/{}".format(args.model)
    snapshot_path = "../run_result/{}_m1_{}_m2_{}".format(args.model, args.model1_type, args.model2_type)
